...
class RegisterPolice_Controller extends GetxController {
  ...
  _CallApi_Register() async {
    try {
      var response = await http.post(Uri.parse(MyUrl.Url + "/RegisterPolice"),
          headers: <String, String>{
            'Content-Type': 'application/json;charset=UTF-8'
          },
          body: jsonEncode(
              {"uphno": number.value.text, "password": password.value.text}));
